...
def main(X_train, X_test, y_train, y_test):
    ...
    for i, test_item in enumerate(X_test):
        ...
        sorted_distances = [(distances[i], i) for i in range(len(distances))]
        ...
        lines.append(np.stack((test_item, X_train[sorted_distances[0][1]])))
        lines.append(np.stack((test_item, X_train[sorted_distances[1][1]])))
        lines.append(np.stack((test_item, X_train[sorted_distances[2][1]])))

        pred_sum = y_train[sorted_distances[0][1]] + y_train[sorted_distances[1][1]] + y_train[sorted_distances[2][1]]
        if pred_sum > 1:
            y_predict[i] = 1
        else:
            y_predict[i] = 0
